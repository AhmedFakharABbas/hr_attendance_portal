...
public isGrievanceMoudle = environment.isGrievanceMoudle;
  username: string;
  key: string;
  type: string;
  hostUrl: string;
  ...
  login() {
    ...
    let laravelAuth = this.auth.getUserDetails(this.username, this.key, this.type, this.app_token, this.os_type, this.app_version,
      ...
      if(this.isGrievanceMoudle){
      let grievanceAuth = this.auth.getUserDetails(this.username, this.key, this.type, this.app_token, this.os_type, this.app_version,
        environment.GrievanceApiURL +'/login');
        forkJoin([ grievanceAuth, ]).subscribe((result) => {
          console.log(result[0]);
          let resData:any=result[0];

          localStorage.setItem('s_u_number', resData.user_id);
          localStorage.setItem('token_nda', JSON.stringify(resData.token));
          localStorage.setItem('e_number', resData.user_id);
          localStorage.setItem('e_code', JSON.stringify(resData.user_id));
          localStorage.setItem('emp_name', JSON.stringify(resData.user_info.name));
          localStorage.setItem('user_info', JSON.stringify(resData.user_info));
          this.router.navigateByUrl('/grievance/emp_portal');
          return;
        ...
        forkJoin([ nodeAuth,laravelAuth ]).subscribe((result) => {
console.log(result);
          ...
          if(nodeObj.success == true) {
            ...
            localStorage.setItem('e_number', nodeObj.result.e_number);
            ...
            let objectOne:any  = result[1];

            if (typeof objectOne != 'undefined' && objectOne.status == 'success') {

              localStorage.setItem('token', JSON.stringify(objectOne.token));
              localStorage.setItem('user_info', JSON.stringify(objectOne.user_info));
