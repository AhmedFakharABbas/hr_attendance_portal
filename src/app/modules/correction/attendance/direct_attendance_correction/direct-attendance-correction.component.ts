...
export class DirectAttendanceCorrectionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.uploadedDirectFileText = new FormControl("");
  }
  ...
  onFileUploadChange(event) {
   // this.resetUploadFile(DirectinputFile);
    this.uploadedFile = undefined;
    if (event.target.files.length > 0) {
      this.uploadedFile = event.target.files[0];
      this.fileName = this.uploadedFile.name;
      this.uploadedDirectFileText.setValue(this.fileName);
    }
  }
  onFileUploadSubmit(DirectinputFile) {
    let data = {};
    ...
    this.attendanceService
      ...
      .subscribe({
        next: (res) => {
          ...
          res.skipRecords!='Skip People codes are[]'?this.sharedService.warningMessage(res.skipRecords):'';
          this.sharedService.successMessage(res.message);
  ...
  //reset form
  resetUploadFile(DirectinputFile) {
    this.uploadedFile = undefined;
    this.fileName = undefined;
    this.uploadedDirectFileText = undefined;
    this.uploadedDirectFileText = new FormControl("");
    this.isShowFileRequired = false;
    DirectinputFile.value=''
