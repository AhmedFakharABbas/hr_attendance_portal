...
(function (factory) {
  ...
  var OrgChart = function (elem, opts) {
    ...
    this.defaultOptions = {
      ...
      'zoomoutLimit': 0.5,
      'hideLeftChild': false,
      'hideRightChild': false,
      'hdieTopChild': false,
      'zoomDuration': '0.1s',
    };
  };

  //var OrgBands = JSON.parse(bands);
  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if ($.type(data) === 'object') {
        if (data instanceof $) { // ul datasource
          this.shrunkView($chart, this.buildJsonDS(data.children()), 0, this.options);

          var completedChart = $.Event('chartcompleted.orgchart');
          this.$chart.trigger(completedChart);
        } else { // local json datasource
          this.shrunkView($chart, this.options.ajaxURL ? data : this.attachRel(data, '00'));

          var completedChart = $.Event('chartcompleted.orgchart');
          this.$chart.trigger(completedChart);
        }
      } else {
        $chart.append('<div id="loader-body" class="loading1 spinner1"><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div></div>');
        $.ajax({
          'url': data,
          'dataType': 'json'
        })
        .done(function(data, textStatus, jqXHR) {
          ...
          if(popupViewer) {
            that.callBuildingFunction( 1, $chart, that.options.ajaxURL ? data : that.attachRel(data, '00'), 0, that.options );
          }
          else {
            that.callBuildingFunction( selectedView, $chart, that.options.ajaxURL ? data : that.attachRel(data, '00'), 0, that.options );
          }

          var completedChart = $.Event('chartcompleted.orgchart');
          that.$chart.trigger(completedChart);
        })
        .fail(function(jqXHR, textStatus, errorThrown) {
          console.log(errorThrown);
        })
        .always(function() {
          $chart.children('.spinner1').remove();
        });
      }
    ...
    callBuildingFunction : function(option, $chart, dataUrl, point, options) {

      if(option == 3 || option == 4) {

        this.departExpandedView( $chart, dataUrl, point, options );

      }
      else {

        this.shrunkView( $chart, dataUrl, point, options );

      }

    },
    buildBands: function($appendTo, bands) {

        var bands_container = "<div class='band_container'>";

        var total_bands = bands.length;

        for( var i = 0; i < total_bands; i++ ){

          bands_container += "<div id='"+bands[i].id+"' class='band_discription' data-nodeId='"+bands[i].id+"'> \n\
                              "+bands[i].band_desc+" \n\
                              </div>";
        ...
        return $appendTo.append( bands_container );

    },
    departExpandedView: function($appendTo, data) {
      var that = this;
      var opts = this.options;
      var level = 0;
      if (data.level) {
        level = data.level; // map child levels
      } else {
        level = data.level = $appendTo.parentsUntil('.orgchart', '.nodes').length + 1; // map only parent level
      }
      // Construct the node
      ...
      if (Object.keys(data).length > 2) {
        var $nodeDiv = this.createNodeDepartmentExpand(data);
      ...
      if (hasChildren) {
        ...
        var isVerticalLayer = that.nextInnerChild(childrenData);//(that.nextInnerChild(childrenData) && (level + 1) >= opts.verticalLevel) ? true : false; //;
...
//          }
            ...
            if (Object.keys(data).length === 2) {
              ...
              $("#"+data.band).append($appendTo.append($upperLines).append(lowerLines).append($nodesLayer));

            } else {

              $("#"+data.band).append($nodeWrapper.append($upperLines).append(lowerLines).append($nodesLayer));
        ...
        $.each(childrenData, function () {
          ...
          that.departExpandedView($nodeCell, this);

        });
      }
    },
    bandExpandedView: function ($appendTo, data) {
      ...
      if (hasChildren) {
        ...
        $.each(childrenData, function () {
          ...
          that.bandExpandedView($nodeCell, this);
    ...
    panEndHandler: function (e) {
      if (e.data.chart.data('panning')) {
        e.data.chart.data('panning', false).css({ 'cursor': 'move',
                                                  'cursor': '-webkit-grab',
                                                  'cursor': 'grab'
                                                }).off('mousemove');
        var panEndHandler = $.Event('panEndHandler');
        e.data.chart.trigger(panEndHandler);
      }
    },
    //
    bindPan: function () {
    ...
    setChartScale: function ($chart, newScale) {
      ...
      if(typeof(opts) == 'undefined') { opts = this.opts; }

      var lastTf = $chart.css('transform');
      var matrix = '';
      var targetScale = 1;
      if (lastTf === 'none') {
        $chart.css({ 'transform': 'scale(' + newScale + ',' + newScale + ')', 'transition-duration': opts.zoomDuration });
      } else {
        matrix = lastTf.split(',');
        if (lastTf.indexOf('3d') === -1) {
          targetScale = Math.abs(window.parseFloat(matrix[3]) * newScale);
          if (targetScale > opts.zoomoutLimit && targetScale < opts.zoominLimit) {
            $chart.css({ 'transform': lastTf + ' scale(' + newScale + ',' + newScale + ')', 'transition-duration': opts.zoomDuration });
          }
        } else {
          targetScale = Math.abs(window.parseFloat(matrix[1]) * newScale);
          if (targetScale > opts.zoomoutLimit && targetScale < opts.zoominLimit) {
            $chart.css({ 'transform': lastTf + ' scale3d(' + newScale + ',' + newScale + ', 1)', 'transition-duration': opts.zoomDuration });
          }
        }
      }
    },
    ...
    hideParent: function ($node) {
      ...
      if ($upperLevel.eq(0).find('.spinner1').length) {
    ...
    stopAjax: function ($nodeLevel) {
      if ($nodeLevel.find('.spinner1').length) {
    ...
    hideChildren: function ($node) {
      ...
      $animatedNodes.addClass('sliding slide-up')
      .eq(0)
      .one('transitionend',
              { 'animatedNodes': $animatedNodes,
                'lowerLevel': $lowerLevel,
                'isVerticalDesc': isVerticalDesc,
                'node': $node }, this.hideChildrenEnd.bind(this));
    },
    //
    showChildrenEnd: function (event) {
      var $node = event.data.node;
      event.data.animatedNodes.removeClass('sliding');
      var $bottomEdge = $node.closest('tr').siblings().eq(2).children().find('.bottomEdge');
      var flag = false;
      flag = this.getNodeState($node, 'children').visible;
      $bottomEdge.removeClass('fa-minus-circle').toggleClass('fa-plus-circle', flag);
      if (this.isInAction($node)) {
        this.switchVerticalArrow($node.children('.bottomEdge'));
      }
    },
    ...
    hideSiblings: function ($node, direction) {
      ...
      if ($nodeContainer.siblings().find('.spinner1').length) {
        $node.closest('.orgchart').data('inAjax', false);
      }
      if (direction) {
        if (direction === 'left') {
    ...
    startLoading: function ($edge) {
      ...
      $edge.parent().append('<div id="loader-body" class="loading1 spinner1"><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div></div>>')
        .children().not('.spinner1').css('opacity', 0.2);
    ...
    isInAction: function ($node) {
      if(typeof($node.children('.edge').attr('class')) === 'undefined' ) {
        return true;
      }
      else {
    ...
    nodeEnterLeaveHandler: function (event) {
      ...
      if (event.type === 'mouseenter') {
        if ($topEdge.length) {
          ...
          $topEdge.toggleClass('fa-minus-circle', !flag).toggleClass('fa-plus-circle', flag);
        }
        ...
        //$node.children('.edge').removeClass('fa-plus-circle fa-minus-circle fa-chevron-right fa-chevron-left');
      }
    },
    //
    ...
    topEdgeClickHandler: function (event) {
      ...
      var vhideTop = $.Event('vhidetop.orgchart');
      that.$chart.trigger(vhideTop);
    },
    //
    bottomEdgeClickHandler: function (event) {
      ...
      if (childrenState.exist) {
        ...
        if (childrenState.visible) {
            ...
            var vhidebottom = $.Event('hidechildnode.orgchart');
            this.$chart.trigger(vhidebottom);
        } else { // show the descendants
            this.showChildren($node);
            var vhidebottom = $.Event('showchildnode.orgchart');
            this.$chart.trigger(vhidebottom);
        }
      ...
      var vhidebottom = $.Event('vhidebottom.orgchart');
      this.$chart.trigger(vhidebottom);
    },
    //
    ...
    dropHandler: function (event) {
      ...
      if (!$dropZone.closest('tr').siblings().length) { // if the drop zone is a leaf node
        $dropZone.append('<i class="edge verticalEdge bottomEdge fa fa-minus-circle"></i>')
        ...
        if (!$dragged.find('.horizontalEdge').length) {
          if(this.opts.hideLeftChild === false && this.opts.hideRightChild === false) {
    ...
    createNode: function (data) {
      ...
      if(data.level_id == 1) {
        ...
        //3. orange
        //4. purple
        first_node = 'inner_node';

      }
      else if(data.level_id == 3) {
        //3. orange
        //4. purple
        first_node = 'orange_node';

      }
      else if(data.level_id == 4) {
        //3. orange
        //4. purple
        first_node = 'purple_node';

      }
      else {
        first_node = 'blue_node';
      }


      // construct the content of node
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '\
                        ' + (data.children ? ' class="node_parent"' : ' class="child_only"') + '>')
        .addClass('node'+ first_node +' ' +(data.className || '') +  (level > opts.visibleLevel ? ' slide-up' : ''));
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        ...
        if (Number(flags.substr(0,1))) {
          if(this.opts.hdieTopChild === false) {
            $nodeDiv.append('<i class="edge verticalEdge topEdge fa"></i>');
          }
        }
        if(Number(flags.substr(1,1))) {

          if(opts.hideLeftChild === false && opts.hideRightChild === false) {
    ...
    createNodeDepartmentExpand: function (data) {
      var that = this;
      var opts = this.options;
      var level = data.level;
      if (data.children) {
      ...
      if(data.group_id.split("_") !== "undefined") {

        splitedString = data.group_id.split("_");
      ...
      if (opts.nodeTemplate) {
        if(splitedString.length > 1) {
          ...
          var title = data.title.split("_");

          var name = data.name.split("_");

          var group_image = data.group_image.split("|");

          var bands = data.band.split("_");

          var level_id = data.level_id.split("_");

          $nodeDiv.append('<h1 class="department_title" title="'+data.companyname+'">'+data.companyname+'</h1>');
          ...
          for(var i = 0; i < splitedString.length; i++) {
            ...
            else if(level_id[i] == 2) {
              //3. orange
              //4. purple
              first_node = 'inner_node';

            }
            else if(level_id[i] == 3) {
              //3. orange
              //4. purple
              first_node = 'orange_node';

            }
            else if(level_id[i] == 4) {
              //3. orange
              //4. purple
              first_node = 'purple_node';
            ...
            if( name[i] != '' && title[i] != '' &&  bands[i] != '') {

              if(number == 1) {

                  $nodeDiv.append('<div id="0000000000000_'+splitedString[i]+'" data-parent="'+data.parentId+'" class="child_only node node_options '+first_node+'">\n\
                  <div class="node_container_1">\n\
                    <div class="action_buttons">\n\
                        <a href="#" class="node_options" id="act_0000000000000_'+splitedString[i]+'"><i class="fa fa-ellipsis-h"></i></a>\n\
                        <nav class="menu" id="men_0000000000000_'+splitedString[i]+'">\n\
                    ...
                    <div class="node_header">\n\
                      <p title="'+name[i]+'">'+name[i]+'</p>\n\
                    </div>\n\
                    <div class="node_content">\n\
                      <div class="node_image_1">\n\
                      <span class="black_image" style="background-image: url('+imageUIServer+'emp_pictures/'+group_image[i]+')"></span>\n\
                      </div>\n\
                      <div class="node_details">\n\
                        <p class="comp_name" title="'+title[i]+'">'+title[i]+'</p>\n\
                        <p class="bands" title="'+bands[i]+'">'+bands[i]+'</p> \n\
                      </div>\n\
                    </div>\n\
                  </div>\n\
                </div>');
              ...
              $nodeDiv.append('<div id="0000000000000_'+splitedString[i]+'" data-parent="'+data.parentId+'" class="child_only node '+first_node+'"> \n\
                                <div class="action_buttons"> \n\
                                <a href="#" class="node_options" id="act_0000000000000_'+splitedString[i]+'"><i class="fa fa-ellipsis-h"></i></a> \n\
                                <nav class="menu" id="men_0000000000000_'+splitedString[i]+'"> \n\
                                  <span class="menu-list"> \n\
                                ...
                                <div class="node_content"> \n\
                                  <p class="node_title" title="'+name[i]+'">'+name[i]+'</p> \n\
                                  <p class="node_position" title="'+title[i]+'">'+title[i]+'</p> \n\
                                </div> \n\
                              </div> \n\
                            </div>');
      ...
      if(splitedString.length > 1) {
          ...
          var $outerDiv = $('<div class="data_wrapper">');

          var $outerInnerDiv = $('<div class="inner_wrapper">');

          $outerDiv.append( $outerInnerDiv.append( $nodeDiv ) );
    ...
    shrunkView: function ($appendTo, data) {
...
//          }
    ...
    buildChildNode: function ($appendTo, data) {
      ...
      this.shrunkView($appendTo, { 'children': data });
    },
    // exposed method
    addChildren: function ($node, data) {
      this.buildChildNode($node.closest('table'), data);
      if (!$node.children('.bottomEdge').length) {
        $node.append('<i class="edge verticalEdge bottomEdge fa fa-minus-circle"></i>');
      }
    ...
    addParent: function ($currentRoot, data) {
      ...
      if (!$currentRoot.children('.topEdge').length && this.opts.hdieTopChild === false) {
    ...
    buildSiblingNode: function ($nodeChart, data) {
      ...
      if ($nodeChart.parent().is('td')) {
        ...
        this.shrunkView($nodeChart.closest('.orgchart'), data);
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      if ($(this).children('.spinner1').length) {
        return false;
      }
      var $chartContainer = this.$chartContainer;
      var $mask = $chartContainer.find('.mask');
      if (!$mask.length) {
        $chartContainer.append('<div class="mask"><div id="loader-body" class="loading1 spinner1"><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div><div class="loading-bar"></div></div>');
      } else {
